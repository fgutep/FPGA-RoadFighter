...
def update_button_state(row, column):
    ...
    buttons[row][column].config(relief=relief)
...
def display_ledsel_window():
    ...
    ledsel_text = tk.Text(ledsel_window, font=('Arial', 12), wrap=tk.WORD, height=500, width=700)
...
label = tk.Label(root, text="8-bit Register-Value generator for FPGA controlled 8^2 led screen", font=('Arial', 15))
texbox = tk.Text(root, font=('Arial', 10), height=1)
...
for i in range(8):
    button_row = []
    for j in range(8):
        button = tk.Button(buttonframe, text=(str(i) + ' ' + str(j)), font=('Arial', 10), command=lambda row=i, col=j: button_click(row, col))
        button.grid(row=i, column=j, sticky=(tk.W + tk.E))
        button_row.append(button)
    buttons.append(button_row)

# "Done" button
done_button = tk.Button(root, text="Done", command=done_button_click, font=('Arial', 12))
